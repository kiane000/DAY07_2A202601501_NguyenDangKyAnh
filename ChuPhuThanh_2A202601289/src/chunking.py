...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Tách sau dấu . ! ? rồi có khoảng trắng/xuống dòng.
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        # Text đã đủ ngắn: không cần tách nữa.
        if len(current_text) <= self.chunk_size:
            return [current_text]

        # Hết separator: cắt cứng theo số ký tự.
        ...
        # "" là separator cuối cùng: cắt cứng.
        if separator == "":
            return [
                current_text[index : index + self.chunk_size]
                for index in range(0, len(current_text), self.chunk_size)
        ...
        # Không có separator này thì thử separator tiếp theo.
        ...
        for part in parts:
            ...
            # Một phần tự nó đã quá dài:
            # flush buffer cũ rồi tách đệ quy part này bằng separator tiếp theo.
            ...
            # Nếu ghép thêm part vẫn vừa chunk thì giữ trong buffer.
            if len(candidate) <= self.chunk_size:
                buffer = candidate
            else:
                # Buffer đã đầy: lưu nó, bắt đầu chunk mới.
                if buffer:
                    chunks.append(buffer)
                buffer = part
